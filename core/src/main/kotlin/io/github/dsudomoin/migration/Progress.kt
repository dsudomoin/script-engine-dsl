...
/**
 * Стратегия прогресс-логирования внутри `forEach`. Передаётся параметром `progress`.
 *
 ...
 * из тела `forEach`-блока). На терминальном `OnError.Fail`-исключении тика **не происходит** —
...
sealed interface Progress {
    /** Авто-режим: каждые ~1000 обработанных item'ов, формат `"forEach progress: X/Y (Z%)"`. Дефолт. */
