...
class CsvFileErrorReporterTest {
    ...
    fun `файл не создаётся до первой ошибки`(@TempDir tmp: Path) {
        ...
        val reporter = CsvFileErrorReporter("SAMPLE-001", csv, log)
    ...
    fun `report пишет CSV строку и полный stacktrace в log`(@TempDir tmp: Path) {
        ...
        assertThat(csvLines[0]).isEqualTo("timestamp,migration,itemRepr,errorClass,errorMessage")
        assertThat(csvLines[1]).contains("SAMPLE-001", "item-123", "IllegalStateException", "broken")
    ...
    fun `includeItem - per-type сериализатор`(@TempDir tmp: Path) {
        val reporter = CsvFileErrorReporter("T", tmp.resolve("e.csv"), tmp.resolve("e.log"))
    ...
    fun `re-run перезаписывает старый errors_csv, не дописывает header посередине`(@TempDir tmp: Path) {
        ...
        CsvFileErrorReporter("T", csv, log).use { reporter ->
        ...
        assertThat(lines[0]).isEqualTo("timestamp,migration,itemRepr,errorClass,errorMessage")
    ...
    fun `truncate больше 500 символов по умолчанию`(@TempDir tmp: Path) {
        ...
        val csv = Files.readString(tmp.resolve("e.csv"))
        val itemField = csv.lines()[1].split(",")[2].trim('"')
